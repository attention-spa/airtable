 ...
 * @arg table {Table}
 */
const updateRecords = (records, table) => table.updateRecordsAsync(records.splice(-50)).then(_ => records.length && updateRecords(records, table));

/** Recursively resolves a record creation queue of any size while respecting Airtable's API caps.
 * @arg records {RecordQueue}
 * @arg table {Table}
 */
const createRecords = (records, table) => table.createRecordsAsync(records.splice(-50)).then(_ => records.length && createRecords(records, table));
